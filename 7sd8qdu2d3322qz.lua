...
local ai_rage_tips = {
  ...
  tips = {
    ...
    ["Quick Peek"] = {
      ...
      {condition = function(data) return data.player_health < 30 end, tip = "ÕËÁÍÓÂ HP, ÌÂ ÔËÍ‡Ú¸, ‰‡ÊÂ Ò ¡˚ÒÚ˚Ï ÔËÍÓÏ!", priority = 3},
...
local phobia = {
	ui = {
		servers = {
			gen = {
                ["ï  PUZO HVH | MODEL SERVER | 16K"] = "45.136.204.145:1488",
                ["ï  eXpidors.Ru | ONLY SCOUT"] = "62.122.215.105:6666",
                ["ï  [hvhserver.xyz] roll fix"] = "62.122.214.55:27015",
                ["ï  eXpidors.Ru - Scout"] = "62.122.215.105:6666",
                ["ï  sippin' on wok mm hvh"] = "46.174.55.52:1488",
                ["ï  War3ft Project"] = "46.174.52.69:27015",
                ["ï  SharkProject | 1x1"] = "46.174.49.147:1488",
                ["ï  SharkProject | MM"] = "37.230.228.148:27015",
                ["ï  PUZO HVH | ONLY SCOUT"] = "37.230.162.58:1488",
                ["ï  LivixProject HVH"] = "185.9.145.159:28423",
                ["ï  Elysium MM HVH | AWP"] = "62.122.214.127:27015",
			},
		...
		header = function(self, group)
			...
			local head = "‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ"
...
pui.macros.ab = "\a6387D0FF"   
			...
			menu = {
				...
				home = {
					configs = {
						...
						save = group.f:button("\f<a>ÓÑÖ\r Save"),
						import = group.f:button("\f<a>ÓÖÅ\r Import"),
						
						load = group.a:button("Load"),
						export = group.a:button("Export"), -- cfgsgsgs

						delete = group.a:button("\aD95148FFÓÑá Delete")
					},
				...
				--servers = {
					...
					--connect = group.a:button("\f<a>ÓÑÇ\r Connect"),
					--copy = group.a:button("\f<a>ÓÑ≤\r Copy ip-address"),

					--space = self:header(group.a),
					--retry = group.a:button("\f<a>ÓÖâ\r Rejoin \f<a>(Retry)"),
				--},

				--stats = {
					--title = group.f:label("\f<d>Statistics"),
					--space = self:header(group.f),
					--loads = group.f:label("\f<a>ÓÑ°\r Total loads:\f<a> "..database.read(databas.load)),
					--kills = group.f:label("\f<a>Óá†\r Total killed:\f<a> "..database.read(databas.kill))
				--},
				...
				antiaim = {
					...
					general = {
						...
						backstab = self.depends(group.a:checkbox("Avoid Backstab"), function()
							return {
								group.a:slider("\n distance backstab", 100, 200, 170, true, "—Å–º", 1),
							}, true
						end),
					...
					antibf = {
						...
						trigger = {
							...
							timer = self.depends(group.a:checkbox("\f<a>ÓÑ°\r Timer"), function()
								return {
									group.a:slider("\f<a>Óáå\r Bruteforce \f<a>reset/on\r ever per", 1, 60, 60, true, "s", 1, {[60] = "1 min"}), -- export
									self:header(group.a)
								}, true
							end),

							round = group.a:checkbox("\f<a>Óáç\r On start \f<a>round"),
							notify = group.a:checkbox("\f<a>Óá£\r Notifys"),
						}
					},
				},
			...
			menu.antiaim.builder.export = group.o:button("\f<a>ÓÖ†\r Export state")
			menu.antiaim.builder.import = group.o:button("\f<a>ÓÖ≠\r Import state")
...
local cals = {
	menu_setup = function(self)
		...
		local f1 = other:to_hex(pr, pg, pb, pa)

		menu.headerr:set("\a"..f1.."‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ‚Äæ")
	...
	logging = {
		...
		create = function(self, arg)
			client.color_log(139, 140, 179, "‚Ä¢ ".. name .. " -\r " .. arg)
		end,
